...
export function UserProfilePage() {
  ...
  const [statView, setStatView] = useState<UserStatView>(null);
  ...
  // Load current user's following list from localStorage
  useEffect(() => {
    if (!user) return;
    try {
  ...
  useEffect(() => {
    (async () => {
      ...
      // Calculate exam readiness with the same algorithm as Dashboard
    ...
    <div className="-mx-4 -mt-4 sm:-mx-6 sm:-mt-6 lg:-mx-8 lg:-mt-8 pb-20">
      ...
      <div className="px-5 pt-16 pb-4">
        ...
        <div className="grid grid-cols-4 gap-2 mt-4">
          {[
            { label: t('community.posts_stat'),     value: userData.postsCount,     icon: 'article',    color: 'text-primary-600', bg: 'bg-primary-50', stat: null as UserStatView },
            { label: t('community.questions_stat'), value: posts.filter(p => p.userId === userId && p.type === 'quiz').length, icon: 'quiz', color: 'text-purple-600', bg: 'bg-purple-50', stat: null as UserStatView },
            { label: t('community.followers_stat'), value: userData.followersCount, icon: 'group',      color: 'text-green-600',   bg: 'bg-green-50',   stat: 'followers' as UserStatView },
            { label: t('community.following_stat'), value: userData.followingCount, icon: 'person_add', color: 'text-blue-600',    bg: 'bg-blue-50',    stat: 'following' as UserStatView },
          ].map(s => (
            <div
              key={s.label}
        ...
        {userData.hideStats ? (
          ...
          <div className="mt-3 bg-gradient-to-br from-primary-50 to-purple-50 rounded-xl p-3 border border-primary-100">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="mr-auto text-[10px] bg-primary-100 text-primary-600 px-1.5 py-0.5 rounded-full font-semibold">
                {t('community.level_label')} {userData.level || 1}
              </span>
            </div>
            ...
            {(statView === 'followers' ? followers : followingList).length === 0 ? (
              <div className="py-10 text-center">
                ...
                <p className="text-xs text-surface-400">
                  {statView === 'followers' ? t('community.no_followers') : t('community.no_following_user')}
                </p>
              </div>
            ) : (
          ...
          <div className="p-4 space-y-2">
            {userPosts.slice(0, 20).map(p => (
              ...
              >
                ...
                <p
                  dir={getTextDir(p.type === 'quiz' ? p.quizQuestion : p.content)}
                ...
                <div className="flex items-center gap-3 mt-2">
                  <span className="text-[10px] text-surface-400 flex items-center gap-0.5">
                    <Icon name="favorite" size={10} className="text-red-400" filled /> {p.likesCount}
                  </span>
                  <span className="text-[10px] text-surface-400 flex items-center gap-0.5">
                    <Icon name="chat_bubble" size={10} /> {p.commentsCount}
                  </span>
                </div>
              </div>
            ))}
            {userPosts.length === 0 && (
              <div className="text-center py-10">
                ...
                <p className="text-xs text-surface-400">
                  {tab === 'posts' ? t('community.no_posts_tab') : t('community.no_quizzes_tab')}
                </p>
              </div>
            )}
          </div>
