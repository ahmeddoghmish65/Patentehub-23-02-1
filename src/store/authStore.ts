...
export const useAuthStore = create<AppState>((set, get) => ({
  ...
  deleteDictSection: async (id) => { const { token } = get(); if (!token) return false; const r = await api.apiSoftDeleteDictSection(token, id); if (r.success) await get().loadDictSections(); return r.success; },
  archiveDictSection: async (id, archive) => { const { token } = get(); if (!token) return false; const r = await api.apiArchiveDictSection(token, id, archive); if (r.success) await get().loadDictSections(); return r.success; },
  restoreDictSection: async (id) => { const { token } = get(); if (!token) return false; const r = await api.apiRestoreDictSection(token, id); if (r.success) await get().loadDictSections(); return r.success; },
  permanentDeleteDictSection: async (id) => { const { token } = get(); if (!token) return false; const r = await api.apiPermanentDeleteDictSection(token, id); if (r.success) await get().loadDictSections(); return r.success; },
  ...
  deleteDictEntry: async (id) => { const { token } = get(); if (!token) return false; const r = await api.apiSoftDeleteDictEntry(token, id); if (r.success) await get().loadDictEntries(); return r.success; },
  archiveDictEntry: async (id, archive) => { const { token } = get(); if (!token) return false; const r = await api.apiArchiveDictEntry(token, id, archive); if (r.success) await get().loadDictEntries(); return r.success; },
  restoreDictEntry: async (id) => { const { token } = get(); if (!token) return false; const r = await api.apiRestoreDictEntry(token, id); if (r.success) await get().loadDictEntries(); return r.success; },
  permanentDeleteDictEntry: async (id) => { const { token } = get(); if (!token) return false; const r = await api.apiPermanentDeleteDictEntry(token, id); if (r.success) await get().loadDictEntries(); return r.success; },
