              ...
              <div className="flex gap-1.5">
                {(['all', 'pending', 'reviewed', 'dismissed'] as const).map(s => (
                  <button key={s} onClick={() => setSearch(s === 'all' ? '' : s)}
                    className={cn('text-[11px] px-2 py-1 rounded-lg border font-medium',
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2 flex-wrap">
                          <span className={cn('text-xs px-2 py-0.5 rounded-full font-medium',
                            ...
                            r.status === 'reviewed' ? 'bg-success-50 text-success-600' : 'bg-surface-100 text-surface-500')}>
                        ...
                        <p className="text-sm text-surface-800 font-medium mb-1">{r.reason}</p>
                        <div className="flex items-center gap-2 text-xs text-surface-400">
                          {reporter && <span className="flex items-center gap-1"><Icon name="person" size={12} /> {reporter.name}</span>}
                          <span>ID: {r.targetId.substring(0, 12)}...</span>
                        </div>
                      </div>
                    </div>
                    {r.status === 'pending' && (
                      <div className="flex gap-2 mt-3">
                        <Button size="sm" onClick={() => store.updateReport(r.id, 'reviewed')}>
                          <Icon name="check" size={16} /> قبول البلاغ
                        </Button>
